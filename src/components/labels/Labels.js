...
import { PAINT_TYPE, PAINT_SCOPE } from '../../utils/constants/config';
import {
...
const Labels = (props) => {
  const { projectId, labels, isFetchingLabels, selectedGroup } = props;
        ...
        setFindingData([
          {
            ...
            [DRAG_TYPE.BOX]: FINDING.filter(
              (it) => it?.annotation_type === PAINT_TYPE.BOUNDING_BOX
            ),
            [DRAG_TYPE.MASK]: FINDING.filter(
              (it) => it?.annotation_type === PAINT_TYPE.MASK
            ),
            [DRAG_TYPE.POLYGON]: FINDING.filter(
              (it) => it?.annotation_type === PAINT_TYPE.POLYGON
            ),
          },
        ]);
      } else {
        ...
        setImpressionData([
          {
            ...
            [DRAG_TYPE.STUDY]: IMPRESSION.filter(
              (it) => it?.scope === PAINT_SCOPE.STUDY
            ),
            [DRAG_TYPE.SERIES]: IMPRESSION.filter(
              (it) => it?.scope === PAINT_SCOPE.SERIES
            ),
            [DRAG_TYPE.IMAGE]: IMPRESSION.filter(
              (it) => it?.scope === PAINT_SCOPE.IMAGE
            ),
          },
        ]);
      } else {
...
export default connect(
  ...
  { actionGetLabelGroups, actionGetLabels }
)(Labels);
