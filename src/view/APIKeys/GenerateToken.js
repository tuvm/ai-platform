  ...
  const onFinish = async () => {
    ...
    if (res && res.data) {
      ...
      notification.success({
        description: t('IDS_API_KEY_CREATE_SUCCESS'),
      });
      actionGetAPIKeys();
      form.resetFields();
    } else {
        ...
        <div className="generate-token-right-side">
          <Form form={form} layout="vertical" onFinish={onFinish}>
            <Form.Item
              ...
              name="key_name"
              rules={[
                { required: true, message: 'Please input your API Key name' },
              ]}
            >
              <Input />
            </Form.Item>
            ...
            <div className="section">
              ...
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: 'Please select at least one API scope',
                  },
                ]}
              >
                <Checkbox.Group>
                  {API_SCOPES &&
                    API_SCOPES.map((item) => (
                      <Checkbox key={item.key} value={item.key}>
                        <span>{item.name}</span>
                        <p className="sub-checkbox">{t(item.description)}</p>
                      </Checkbox>
                    ))}
                </Checkbox.Group>
              </Form.Item>
